...
import { CheckPasswordConditions, checkPasswordConditions } from '../../utils/auth';
...
const PasswordConditions: React.FC<PasswordConditionsProps> = ({ 
    ...
    // Calculate progress
    const validConditionsCount = conditionsList.filter(condition => condition.isValid).length;
    const progressPercentage = validConditionsCount * (100 / conditionsList.length);

    // Determine progress color
    const progressColor = conditions.isValid && (!passwordMatchCheck || password === matchPassword)
      ? 'bg-green-500' 
      : password.length > 0 
        ? 'bg-yellow-500' 
        : 'bg-gray-300';

    if (conditions.isValid && (!passwordMatchCheck || password === matchPassword)) {
        return null;
    }
    ...
    return (
      <div className="password-conditions my-2 space-y-2">
        ...
        <div className="text-sm">
          <div className="text-gray-600 mb-2 font-medium">Password Requirements:</div>
