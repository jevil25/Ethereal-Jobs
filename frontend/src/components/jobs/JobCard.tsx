...
const JobCard: React.FC<JobCardProps> = ({ job, redirect = true }) => {
  ...
  const formatSalary = () => {
    if (!job.min_amount && !job.max_amount && !job.salary_with_currency)
  ...
  const redirectToJobPage = (e: React.MouseEvent) => {
    ...
    if (!redirect) return;
    isJobPage
      ? window.open(job.job_url_direct || job.url)
    ...
    <>
      <Card className="hover:shadow-md transition-all duration-300 border border-gray-200 overflow-hidden group">
        ...
        <CardHeader className="py-2 px-4 flex items-start justify-between gap-3">
          <div className="flex gap-3 items-center w-full flex-col md:flex-row">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {job.is_remote && (
                    <Badge
                      variant="outline"
                      className="text-xs bg-blue-50 text-blue-700 border-blue-200 py-0 px-1.5"
                    >
                      Remote
                    </Badge>
                  )}
            ...
            {job.application_status !== ApplicationStatus.Pending && (
              <div className="mr-auto md:ml-auto">
                <Badge
                  className={`font-medium text-sm px-2 py-1 min-w-14 text-center ${getApplicationStatusColor(job.application_status)}`}
                >
          ...
          {job.match_score && (
            <Badge
              className={`font-medium text-sm px-2 py-1 min-w-14 text-center ${getMatchScoreColor(job.match_score)}`}
            >
              <Star className="h-3 w-3 mr-1 fill-current" /> {job.match_score}%
              Match Score
            </Badge>
          )}
        </CardHeader>
        ...
        <CardContent className="py-2 px-4 space-y-2">
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
            ...
            <div className="flex items-center text-gray-700">
              ...
              <span className="font-medium truncate">
                {job.job_level ||
                  getBasedOnJobRequiredYears(job.job_required_years)}
              </span>
            </div>
          </div>
              ...
              {job.tfidf_similarity !== undefined && (
                <div className="inline-flex items-center text-xs bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100">
                  ...
                  <span className="ml-1 text-blue-700 font-medium">
                    {Math.round(job.tfidf_similarity)}% {getScoreLabel("tfidf")}
                  </span>
                </div>
              )}
              ...
              {job.semantic_similarity !== undefined && (
                <div className="inline-flex items-center text-xs bg-purple-50 px-2 py-0.5 rounded-full border border-purple-100">
                  ...
                  <span className="ml-1 text-purple-700 font-medium">
                    {Math.round(job.semantic_similarity)}%{" "}
                    {getScoreLabel("semantic")}
                  </span>
                </div>
              )}
              ...
              {job.experience_match_score !== undefined && (
                <div className="inline-flex items-center text-xs bg-amber-50 px-2 py-0.5 rounded-full border border-amber-100">
                  ...
                  <span className="ml-1 text-amber-700 font-medium">
                    {Math.round(job.experience_match_score)}%{" "}
                    {getScoreLabel("experience")}
                  </span>
                </div>
              )}
            </div>
            ...
            <div>
              {job.job_function && (
                <div>
                  <Badge
                    variant="outline"
                    className="bg-gray-50 border-gray-200 text-gray-700 font-medium text-xs capitalize"
                  >
                    {job.job_function}
                  </Badge>
                </div>
      ...
      {openIsAppliedModal && (
        <Dialog open={openIsAppliedModal} onOpenChange={setOpenIsAppliedModal}>
          <DialogContent className="sm:max-w-md p-6">
            <DialogHeader>
              ...
              <DialogDescription className="text-gray-600 mt-1">
                Please confirm if you've submitted an application for this
                position
              </DialogDescription>
            </DialogHeader>
