          ...
          return (
            <div className="flex justify-between items-center w-full">
              ...
              <div className="flex items-center space-x-2 justify-center">
                <span className="text-sm font-medium">{month.toLocaleString("default", { month: "long" })}</span>
                <select
                  className="text-sm font-medium bg-transparent border-none outline-none flex items-center"
                  value={month.getFullYear()}
                  onChange={(e) => setMonth(new Date(parseInt(e.target.value), month.getMonth()))}
                >
...
function CalendarForm({ value, setValue, disabled }: { value: Date | undefined; setValue: (date: Date | undefined) => void, disabled?: boolean }) {
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant="outline" className="w-[240px] pl-3 text-left font-normal" disabled={disabled}>
